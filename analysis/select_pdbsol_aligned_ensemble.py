...
def main():
 ...
 prior=json.loads(args.base_report.read_text())['reports'];base_entry=max(prior,key=lambda row:row['uncalibrated']['auroc']);base_runs=base_entry['runs'];targets,_,entities=load(base_runs[0]);base=np.mean([load(run)[1] for run in base_runs],0);candidates=[]
 ...
 report={'selection_data':'strict-validation only','test_evaluated':False,'base_report':str(args.base_report),'base_runs':base_runs,'base_weight':float(1-weights.sum()),'candidate_weights':{str(run):float(weight) for run,weight in zip(args.candidate,weights)},'uncalibrated':binary_metrics(targets,logits),'temperature':temperature,'calibrated':binary_metrics(targets,calibrated),'mcc_threshold':threshold,'mcc_at_selected_threshold':mcc,'entity_count':int(len(entities))}
