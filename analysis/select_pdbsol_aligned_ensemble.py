...
def main():
 parser=argparse.ArgumentParser();parser.add_argument('--base-report',type=Path,required=True);parser.add_argument('--candidate',type=Path,action='append',required=True);parser.add_argument('--objective',choices=('auroc','auprc'),default='auroc');parser.add_argument('--output',type=Path,required=True);args=parser.parse_args()
 ...
 score=(lambda truth,prediction:roc_auc_score(truth,prediction)) if args.objective=='auroc' else (lambda truth,prediction:average_precision_score(truth,prediction));weights=np.zeros(len(candidates));best=score(targets,base);grid=np.linspace(0,.25,26)
 for _ in range(6):
  changed=False
  for index in range(len(weights)):
   ...
   for value in grid:
    ...
    logits=(1-proposed.sum())*base+sum(weight*prediction for weight,prediction in zip(proposed,candidates));value_score=score(targets,logits)
    if value_score>winner[0]+1e-12:winner=(value_score,value)
