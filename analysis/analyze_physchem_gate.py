...
def main():
 ...
 with torch.inference_mode():weights=torch.softmax(gate(torch.from_numpy(descriptors)),1).numpy()
 entropy=-(weights*np.log(np.clip(weights,1e-12,None))).sum(1);winners=weights.argmax(1);report={'selection_data':'strict-validation only','test_evaluated':False,'run':str(args.run),'entity_count':int(len(entities)),'experts':experts,'mean_weights':weights.mean(0).tolist(),'std_weights':weights.std(0).tolist(),'argmax_fraction':np.bincount(winners,minlength=experts).astype(float).tolist(),'mean_entropy':float(entropy.mean()),'maximum_entropy':float(np.log(experts)),'effective_experts':float(np.exp(entropy.mean()))};report['argmax_fraction']=(np.asarray(report['argmax_fraction'])/len(winners)).tolist();args.output.parent.mkdir(parents=True,exist_ok=True);args.output.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n');print(json.dumps(report,indent=2,sort_keys=True))
