...
def main():
 ...
 weights=np.zeros(len(candidate_values));best=float(spearmanr(targets,base).statistic)
 for _ in range(6):
  changed=False
  for index in range(len(weights)):
   ...
   for value in np.linspace(0,.3,301):
    ...
    score=float(spearmanr(targets,prediction).statistic)
 ...
 slope,intercept=np.polyfit(prediction,targets,1);affine=slope*prediction+intercept;report={'selection_data':'strict-validation only','test_evaluated':False,'objective':'spearman','base_report':str(args.base_report),'candidate_weights':{str(run):float(weight) for run,weight in zip(args.candidate,weights)},'candidate_entities':{str(run):int(len(selected)) for run,selected in zip(args.candidate,candidate_positions)},'fallback_rule':'Keep the frozen base prediction for entities absent from each partial candidate.','metrics':regression_metrics(targets,prediction),'affine_slope':float(slope),'affine_intercept':float(intercept),'affine_metrics':regression_metrics(targets,affine),'entity_count':int(len(entities))};args.output.parent.mkdir(parents=True,exist_ok=True);args.output.write_text(json.dumps(report,indent=2,sort_keys=True)+'\n');print(json.dumps(report,indent=2,sort_keys=True))
