...
class LatentEndpointTests(unittest.TestCase):
 ...
 def test_assay_residual_starts_as_exact_shared_latent_model(self):
  torch.manual_seed(13);base=LatentEndpointModel(12,16,0,3);residual=LatentEndpointModel(12,16,0,3,assay_residual_dimension=4,assay_residual_scale=.25);residual.encoder.load_state_dict(base.encoder.state_dict());residual.raw_slopes.data.copy_(base.raw_slopes);residual.intercepts.data.copy_(base.intercepts);features=torch.randn(6,12);endpoint=torch.tensor([0,1,2,0,1,2]);self.assertTrue(torch.equal(base(features,endpoint),residual(features,endpoint)));self.assertEqual(float(residual.assay_residual_penalty().detach()),0.)
 def test_assay_residual_is_bounded_and_source_specific(self):
  model=LatentEndpointModel(12,16,0,3,assay_residual_dimension=2,assay_residual_scale=.2);model.assay_residual_weights.data[0].fill_(10);features=torch.randn(4,12);endpoint=torch.zeros(4,dtype=torch.long);shared=model.slopes[endpoint]*model.latent(features)+model.intercepts[endpoint];self.assertTrue(torch.all((model.observation_logits(features,endpoint)-shared).abs()<=.200001));self.assertGreater(float(model.assay_residual_penalty().detach()),0.)
 def test_assay_residual_requires_dimension_and_scale(self):
  with self.assertRaisesRegex(ValueError,'both'):LatentEndpointModel(12,16,0,3,assay_residual_dimension=4)
