...
class ConfidencePropagationTests(unittest.TestCase):
 def test_rsa_uses_inverse_transformed_spatial_scalar(self):
  model=GeometryLateFusion(confidence_mode='propagated_moe',rsa_mean=.3,rsa_std=.2);residue=torch.zeros(2,152);residue[:,63]=torch.tensor([-1.,2.]);residue[:,21]=99
  torch.testing.assert_close(model.residue_rsa(residue),torch.tensor([.1,.7]))
