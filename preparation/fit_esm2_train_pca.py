...
def main():
 p=argparse.ArgumentParser();p.add_argument('--entities',type=Path,required=True);p.add_argument('--observation-split',type=Path,required=True);p.add_argument('--embedding-dir',type=Path,required=True);p.add_argument('--structure-status',type=Path,required=True);p.add_argument('--output',type=Path,required=True);p.add_argument('--dimensions',type=int,default=256);a=p.parse_args()
