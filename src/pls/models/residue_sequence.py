 ...
 def forward(self,global_embedding,residues,mask):
  ...
  if self.global_segments==1:global_encoded=self.global_encoder(global_embedding)
   ...
   if self.global_segment_fusion=='logit_mixture':
    expert_logits=torch.stack([head(torch.cat((encoded[:,i],pooled),1)).squeeze(-1) for i,head in enumerate(self.expert_heads)],1);return (expert_logits*torch.softmax(self.global_segment_logits,0)[None]).sum(1)
