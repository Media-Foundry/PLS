...
class ResidueSequenceRegressor(nn.Module):
 def __init__(self,global_dimension=1280,residue_dimension=256,hidden_dimension=256,representation_dimension=256,dropout=.2,pooling='attention',fusion='concat',global_segments=1,global_segment_fusion='weighted_sum',tcn_bottleneck=None,tcn_dilations=(1,2,4),segment_window=32,segment_layers=2,descriptor_dimension=0,physchem_experts=0):
  ...
  if pooling in ('conditioned_attention','tcn_conditioned_attention','shift_tcn_conditioned_attention','gated_shift_tcn_conditioned_attention','segment_transformer') and global_segments>1:raise ValueError('conditioned pooling requires global_segments=1; fuse segments before conditioning')
 ...
 def forward(self,global_embedding,residues,mask):
  ...
  if self.physchem_experts:
   combined=torch.cat(features,1);self.last_expert_logits=torch.stack([head(combined).squeeze(-1) for head in self.physchem_heads],1);self.last_expert_weights=torch.softmax(self.physchem_gate(global_embedding[:,-self.descriptor_dimension:]),1);return (self.last_expert_logits*self.last_expert_weights).sum(1)
