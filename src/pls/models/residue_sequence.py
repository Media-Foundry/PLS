...
class ResidueSequenceRegressor(nn.Module):
 def __init__(self,global_dimension=1280,residue_dimension=256,hidden_dimension=256,representation_dimension=256,dropout=.2,pooling='attention',fusion='concat',global_segments=1,global_segment_fusion='weighted_sum',tcn_bottleneck=None,tcn_dilations=(1,2,4),segment_window=32,segment_layers=2):
  ...
  if pooling in ('conditioned_attention','tcn_conditioned_attention','shift_tcn_conditioned_attention','segment_transformer') and global_segments>1:raise ValueError('conditioned pooling requires global_segments=1; fuse segments before conditioning')
