...
class ResidueSequenceRegressor(nn.Module):
 def __init__(self,global_dimension=1280,residue_dimension=256,hidden_dimension=256,representation_dimension=256,dropout=.2,pooling='attention',fusion='concat',global_segments=1,global_segment_fusion='weighted_sum'):
  ...
  if global_segment_fusion not in ('weighted_sum','concat'):raise ValueError('global segment fusion must be weighted_sum or concat')
  if global_segments==1 and global_segment_fusion!='weighted_sum':raise ValueError('global segment concat requires multiple segments')
 ...
 def forward(self,global_embedding,residues,mask):
  ...
  if self.global_segments==1:global_encoded=self.global_encoder(global_embedding)
   ...
   segments=global_embedding.chunk(self.global_segments,1);encoded=torch.stack([encoder(segment) for encoder,segment in zip(self.global_segment_encoders,segments)],1);global_encoded=encoded.flatten(1) if self.global_segment_fusion=='concat' else (encoded*torch.softmax(self.global_segment_logits,0)[None,:,None]).sum(1)
