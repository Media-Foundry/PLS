...
class ResidueSequenceRegressor(nn.Module):
 ...
 def forward(self,global_embedding,residues,mask):
  ...
  if self.physchem_experts:
   combined=torch.cat(features,1);self.last_expert_logits=torch.stack([head(combined).squeeze(-1) for head in self.physchem_heads],1);weights=torch.softmax(self.physchem_gate(global_embedding[:,-self.descriptor_dimension:])/self.physchem_gate_temperature,1)
   if self.physchem_top_k:
    selected=torch.topk(weights,self.physchem_top_k,dim=1).indices;mask=torch.zeros_like(weights).scatter_(1,selected,1);weights=weights*mask;weights=weights/weights.sum(1,keepdim=True).clamp_min(1e-8)
