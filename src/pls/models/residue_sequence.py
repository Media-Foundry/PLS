...
class ResidueSequenceRegressor(nn.Module):
 ...
 def forward(self,global_embedding,residues,mask):
  ...
  if self.physchem_experts:
   combined=torch.cat(features,1);self.last_expert_logits=torch.stack([head(combined).squeeze(-1) for head in self.physchem_heads],1);self.last_expert_weights=torch.softmax(self.physchem_gate(global_embedding[:,-self.descriptor_dimension:])/self.physchem_gate_temperature,1);return (self.last_expert_logits*self.last_expert_weights).sum(1)
