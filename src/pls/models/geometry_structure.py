 ...
 def pool_surface_patches(self,z,patch,plddt,rsa,mask,component_ids,confidence):
  ...
  for batch_index in range(len(z)):
   ...
   for category in range(component_ids.shape[-1]):
    ...
    size=torch.zeros(count,device=z.device,dtype=z.dtype).index_add_(0,inverse,torch.ones_like(weights,dtype=z.dtype));rsa_sum=torch.zeros_like(size).index_add_(0,inverse,rsa[batch_index,valid].to(z.dtype));confidence_sum=torch.zeros_like(size).index_add_(0,inverse,plddt[batch_index,valid].to(z.dtype));hydropathy=patch[batch_index,valid,0].to(z.dtype);hydropathy_sum=torch.zeros_like(size).index_add_(0,inverse,hydropathy);confidence_min=torch.full_like(size,float('inf')).scatter_reduce_(0,inverse,plddt[batch_index,valid].to(z.dtype),reduce='amin',include_self=True);hydropathy_max=torch.full_like(size,-float('inf')).scatter_reduce_(0,inverse,hydropathy,reduce='amax',include_self=True);category_onehot=torch.nn.functional.one_hot(torch.full((count,),category,device=z.device),component_ids.shape[-1]).to(z.dtype);descriptors=torch.cat((torch.stack((torch.log1p(size)/5,rsa_sum/10,rsa_sum/size,confidence_sum/size,confidence_min,hydropathy_sum/size,hydropathy_max,denominator/size),-1),category_onehot),-1);tokens.append(self.surface_patch_token(torch.cat((embedding,descriptors),-1)));scores.append(rsa_sum*confidence_sum/size.clamp_min(1))
   if tokens:
