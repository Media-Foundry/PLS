...
class GeometryLateFusion(nn.Module):
 def __init__(self,seq_dimension=1280,input_dimension=152,hidden_dimension=256,representation_dimension=256,dropout=.15,layers=1,pooling='attention',global_dimension=0,use_sequence_separation=False,fusion='concat',confidence_mode='legacy'):
  ...
  branches=(5 if fusion in ('cross_attention','global_query_pooling') else 2)+(1 if global_dimension else 0);self.head=nn.Sequential(nn.LayerNorm(representation_dimension*branches),nn.Linear(representation_dimension*branches,representation_dimension),nn.GELU(),nn.Dropout(dropout),nn.Linear(representation_dimension,1))
 def forward(self,sequence,residue,plddt,patch,mask,neighbors,distances,global_features=None):
  ...
  z=self.input(residue)*(confidence[...,None] if confidence is not None else mask[...,None])
