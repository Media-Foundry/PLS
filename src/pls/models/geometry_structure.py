...
class GeometryLateFusion(nn.Module):
 ...
 def forward(self,sequence,residue,plddt,patch,mask,neighbors,distances,global_features=None):
  ...
  quality=torch.stack(((plddt*mask).sum(1)/count,((plddt<.7)&mask).sum(1)/count,torch.log1p(count)/10),1);pooled=pooled*self.gate(quality);branches=[self.sequence(sequence),pooled]
