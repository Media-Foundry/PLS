...
class GeometryLateFusion(nn.Module):
 ...
 def pool_surface_patches(self,z,patch,plddt,rsa,mask,component_ids,confidence):
  batch_size,residues,categories=component_ids.shape;expanded_mask=mask[...,None].expand_as(component_ids);valid=(component_ids>=0)&expanded_mask;batch_ids=torch.arange(batch_size,device=z.device)[:,None,None].expand_as(component_ids);category_ids=torch.arange(categories,device=z.device)[None,None,:].expand_as(component_ids);group_ids=((batch_ids*categories+category_ids)*(residues+1)+component_ids.clamp_min(0))[valid];unique,inverse=torch.unique(group_ids,sorted=True,return_inverse=True);groups=len(unique);meta=unique//(residues+1);group_batches=meta//categories;group_categories=meta%categories
