...
class BalancedLengthBatchSampler(Sampler):
 def __init__(self,labels,lengths,batch_size,seed,entity_ids=None,pool_factor=20):
  self.labels,self.lengths,self.batch_size,self.seed,self.pool_factor=labels,lengths,batch_size,seed,pool_factor;self.epoch=0;entity_ids=np.arange(len(labels)) if entity_ids is None else np.asarray(entity_ids);_,inverse,counts=np.unique(entity_ids,return_inverse=True,return_counts=True);entity_weight=1/counts[inverse];class_mass=np.bincount(np.asarray(labels,dtype=int),weights=entity_weight,minlength=2);self.weights=torch.tensor([entity_weight[i]/class_mass[int(label)] for i,label in enumerate(labels)],dtype=torch.double)
 def __len__(self): return (len(self.labels)+self.batch_size-1)//self.batch_size
 def __iter__(self):
  g=torch.Generator().manual_seed(self.seed+self.epoch);self.epoch+=1;indices=torch.multinomial(self.weights,len(self.weights),replacement=True,generator=g).tolist();pool=self.batch_size*self.pool_factor
