...
class GVPData(Data):
 ...
 def __getitem__(self,i):
  ...
  return (*base[:-1],torch.from_numpy(np.array(self._vectors[lo:hi],dtype=np.float32,copy=True)),torch.from_numpy(np.array(self._coords[lo:hi],copy=True)),base[-1])
def collate(batch):
...
def main():
 ...
 if c.get('evaluate_test',False) or a.allow_test_evaluation:p.error('GVP validation selection never evaluates test')
 ...
 residue_dimension=152+int(json.loads((residue_esm/'pca_metadata.json').read_text())['shape'][1]);device=torch.device('cuda:0');model=GVPStructureFusion(esm.shape[1],residue_dimension,mc['scalar_dimension'],mc['vector_dimension'],mc['representation_dimension'],mc['dropout'],mc['layers'],mc.get('fusion','interaction')).to(device);decay=float(tr.get('ema_decay',0));ema=copy.deepcopy(model).eval().requires_grad_(False) if decay else None;opt=torch.optim.AdamW(model.parameters(),lr=tr['learning_rate'],weight_decay=tr['weight_decay'],fused=tr.get('fused_optimizer',False));writer=SummaryWriter(a.run_dir/'tensorboard');best=-1;stale=0;history=[]
