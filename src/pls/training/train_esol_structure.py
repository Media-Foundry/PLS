...
def infer(model,loader,device,amp=False):
 ...
 with torch.inference_mode():
  for seq,res,p,patch,mask,neighbors,distances,global_features,y in loader:
   values=[v.to(device,non_blocking=True) for v in (seq,res,p,patch,mask,neighbors,distances,global_features)]
...
def main():
 ...
 with open(d['observation_split'],newline='',encoding='utf-8') as h:
  for r in csv.DictReader(h):
   if r['source_dataset']==SOURCE:rows[r['split']].append((index[r['sequence_sha256']],r['sequence_sha256'],float(r['target_value'])))
 ...
 device=torch.device('cuda:0');residue_dimension=152+44+int(json.loads((residue_esm/'pca_metadata.json').read_text())['shape'][1]);global_dimension=descriptors.shape[1] if descriptors is not None else 0;model=GeometryLateFusion(esm.shape[1],residue_dimension,mc['hidden_dimension'],mc['representation_dimension'],mc['dropout'],mc.get('geometry_layers',1),mc['pooling'],global_dimension,mc.get('use_sequence_separation',False),mc.get('fusion','concat')).to(device);decay=float(tr.get('ema_decay',0));ema=copy.deepcopy(model).eval().requires_grad_(False) if decay else None;opt=torch.optim.AdamW(model.parameters(),lr=tr['learning_rate'],weight_decay=tr['weight_decay'],fused=tr.get('fused_optimizer',False));writer=SummaryWriter(a.run_dir/'tensorboard');best=-2;stale=0;history=[]
 for epoch in range(1,tr['epochs']+1):
  started=time.monotonic();model.train();total=count=0
  for seq,res,pconf,patch,mask,neighbors,distances,global_features,y in train:
   seq,res,pconf,patch,mask,neighbors,distances,global_features,y=[v.to(device,non_blocking=True) for v in (seq,res,pconf,patch,mask,neighbors,distances,global_features,y)];opt.zero_grad(set_to_none=True)
   with torch.autocast('cuda',dtype=torch.bfloat16,enabled=tr.get('amp_bfloat16',True)):pred=model(seq,res,pconf,patch,mask,neighbors,distances,global_features);loss=nn.functional.smooth_l1_loss(pred,y,beta=.1)+float(tr.get('rank_weight',0))*rank_loss(pred,y)
