...
def main():
 ...
 device=torch.device('cuda:0');model=ResidueSequenceRegressor(global_esm.shape[1],shape[1],mc['hidden_dimension'],mc['representation_dimension'],mc['dropout'],mc['pooling'],mc.get('fusion','concat'),mc.get('global_segments',1)).to(device);decay=float(tr.get('ema_decay',0));ema=copy.deepcopy(model).eval().requires_grad_(False) if decay else None;opt=torch.optim.AdamW(model.parameters(),lr=tr['learning_rate'],weight_decay=tr['weight_decay']);writer=SummaryWriter(a.run_dir/'tensorboard');best=-2;stale=0;history=[]
